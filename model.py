...
def model_fn(features, labels, mode, params):
  ...
  with tf.compat.v1.variable_scope("model"):
    ...
    par_pos_embs = tf.compat.v1.get_variable("par_pos_embs", shape=[config.max_sent_len * config.sent_window, config.positional_embs_size],
                                        initializer=tf.compat.v1.keras.initializers.glorot_normal(), dtype = tf.float32)
    sent_pos_embs = tf.compat.v1.get_variable("sent_pos_embs", shape=[config.max_sent_len, config.positional_embs_size],
                                        initializer=tf.compat.v1.keras.initializers.glorot_normal(), dtype = tf.float32)
    ...
    with tf.compat.v1.variable_scope("tok_trans"):
    ...
    with tf.compat.v1.variable_scope("sent_trans"):
    ...
    with tf.compat.v1.variable_scope("seg_classifier"):
      w_seg = tf.compat.v1.get_variable("w_seg", shape=[config.vecs_dim + 2*config.positional_embs_size, 2],
                                       initializer=tf.compat.v1.keras.initializers.glorot_normal(), dtype = tf.float32)
      b_seg = tf.compat.v1.get_variable("b_seg", initializer=tf.zeros(2), dtype = tf.float32)

    seg_probs = tf.nn.softmax(tf.add(tf.tensordot(sent_reps_true, w_seg, axes = [[2], [0]]), b_seg))
    seg_labs = tf.map_fn(lambda x: tf.cond(pred=tf.equal(x, 1), true_fn=lambda: tf.constant([0, 1], dtype=tf.float32),
                                  false_fn=lambda: tf.constant([1, 0], dtype=tf.float32)), tf.cast(features["seg_lab"], tf.float32))
    seg_labs = tf.reshape(seg_labs, [config.batch_size, config.sent_window, -1])

    # coherence contrast
    print("Coherence regressor...")    
    with tf.compat.v1.variable_scope("coherence_regressor"):
      w_coh = tf.compat.v1.get_variable("w_coh", shape=[config.vecs_dim + 2*config.positional_embs_size],
                                       initializer=tf.compat.v1.keras.initializers.glorot_normal(), dtype = tf.float32)
      b_coh = tf.compat.v1.get_variable("b_coh", initializer=tf.zeros(1), dtype = tf.float32)
    ...
    loss_seg = -1 * tf.reduce_sum(input_tensor=tf.multiply(tf.math.log(seg_probs), seg_labs))
    if config.MODEL_TYPE == "cats":
      loss_coh = tf.reduce_sum(input_tensor=tf.maximum(tf.constant(0, dtype = tf.float32), tf.constant(params["coherence_hinge_margin"], dtype = tf.float32) - scores_diff))
      loss_sum = loss_seg + loss_coh

    tf.compat.v1.summary.scalar('seg_loss', loss_seg)
    if config.MODEL_TYPE == "cats":
      tf.compat.v1.summary.scalar('coh_loss', loss_coh)

    # train_operations
    print("Defining train operations...")     
    if mode == tf.estimator.ModeKeys.TRAIN:
      print("Defining segmentation optimization...")  
      optimizer_seg = tf.compat.v1.train.AdamOptimizer(learning_rate=params["learning_rate"])
      train_op_seg = optimizer_seg.minimize(loss_seg, global_step=tf.compat.v1.train.get_global_step())

      if config.MODEL_TYPE == "cats":
        print("Defining coherence optimization...")
        optimizer_coh = tf.compat.v1.train.AdamOptimizer(learning_rate=params["learning_rate"])
        train_op_coh = optimizer_coh.minimize(loss_coh, global_step=tf.compat.v1.train.get_global_step())
      ...
      preds = tf.argmax(input=seg_probs, axis = 0)
