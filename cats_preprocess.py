...
def preprocess(input_dir, output_dir, vocabulary, train=False, ssplit=False):
    ...
    res = serializer.create_instances(input_dir,
                                      vocabulary,
                                      os.path.join(output_dir, config.data_preprocessed_records_file),
                                      test=test_mode,
                                      title_start=config.seg_start,
                                      ssplit=(ssplit is True))
    pickle.dump(res, open(os.path.join(output_dir, config.data_preprocessed_blocks_file), "wb+"))
...
if __name__ == "__main__":
    ...
    preprocess(config.data_original_input_dir, config.data_preprocessed_output_dir, vocabulary, train=True if args.train == 1 else False, ssplit=True if args.ssplit == 1 else False)
