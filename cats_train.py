...
params = {"padding_value": vocab["<PAD>"], "wembs": embs, "vocab": vocab, "coherence_hinge_margin": 1,
          "learning_rate": 0.0001}
estimator = tf.estimator.Estimator(model_fn=model.model_fn, config=rconf, params=params)

print("Training the model...")
res = estimator.train(
    input_fn=lambda: get_data.get_data(f'{config.data_preprocessed_output_dir}{config.data_preprocessed_records_file}',
                                       is_train=True, epochs=config.EPOCHS))
# %%
